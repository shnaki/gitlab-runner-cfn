...
- S3 キャッシュ設定（`CacheBucketName` 非空時）→ `runners.cache`（プリフィックスは `CachePathPrefix` で指定）
...
|---|---|---|
| `/var/log/user-data.log` | `/<スタック名>/server` | `<instance-id>/user-data` |
| `/var/log/gitlab-runner.log` | `/<スタック名>/server` | `<instance-id>/gitlab-runner` |
