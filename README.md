...
### コンソールから手動デプロイしたい場合（Quick-Create Link）

CLI が使えない環境や、コンソールから手動でスタックを作成したい場合は、パラメータ事前入力済みの Quick-Create Link を生成できる。

```bash
# テンプレートを S3 にアップロードしておく
aws s3 cp gitlab-runner-iam.yaml s3://YOUR_BUCKET/
aws s3 cp gitlab-runner.yaml s3://YOUR_BUCKET/

# Quick-Create Link を生成（TEMPLATE_S3_BUCKET は必須）
make quick-create-link TEMPLATE_S3_BUCKET=YOUR_BUCKET

# プレフィックスがある場合
make quick-create-link TEMPLATE_S3_BUCKET=YOUR_BUCKET TEMPLATE_S3_PREFIX=cfn/v1

# Markdown 形式で出力（README への貼り付け用）
make quick-create-link TEMPLATE_S3_BUCKET=YOUR_BUCKET FORMAT=markdown
```

出力された URL をブラウザで開くと、`parameters.json` の値が事前入力された状態で CloudFormation のスタック作成画面が開く。
`RegistrationToken`（NoEcho）は URL から除外されるため、コンソール上で手入力が必要。
詳細な手順は [docs/console-deploy.md](docs/console-deploy.md) を参照。
...
ログは `/${StackName}/server` ロググループに収集される。保持日数は `CloudWatchLogsRetentionDays` で制御（デフォルト 30 日）。

```bash
# ログの確認（AWS Console の CloudWatch Logs を使うか CLI で）
aws logs tail /gitlab-runner/server --follow --region ap-northeast-1
```
